...
def main():
    ...
    plt.figure()
    plt.hist([pcr_gross_errors, lasso_gross_errors],rwidth=0.9,bins=[i for i in range(0,6)],align='left',label=['PCR','LASSO'])
    plt.legend(loc='upper right')
    plt.title('PCR vs. LASSO Distribution of Gross Errors (1000 runs, 80:20 training:test)')
    plt.ylabel('Frequency')
    plt.xlabel('Count of Gross Errors')
    plt.xticks([i for i in range(0,6)],[str(i) for i in range(0,6)])
    plt.show()
